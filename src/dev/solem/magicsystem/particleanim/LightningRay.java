	...
	*/
	...
	public void playAnimation(Location location, Vector direction) {
		...
		for(int i=0; i<20; i++) {
			...
			drawParticleLine(sublocation.add(rdir), rdirinv, rdirinv.length(), samples/2);
			sublocation = sublocation.add(direction.multiply(2.0));
		}
	}
	public void drawParticleLine(Location a, Vector dir, double len, int n) {
		...
		for(int i=0; i<n; i++) {
			...
			delta.add(dir.normalize().multiply(((double)len)/n));
		}
	}
